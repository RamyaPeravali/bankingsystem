package com.ivy.common.controller;
...
import com.ivy.common.JDBC.JDBCHelper;
import com.ivy.common.clientWebServer.JerseyClientPost;
import com.ivy.common.model.CreditCardDetails;
import com.ivy.common.model.Transaction;
import com.ivy.common.model.User;
import com.ivy.common.model.displDate;
import com.ivy.common.model.listTransactions;
import com.ivy.common.model.authCreditCard;
...
//-----------------------------------------------------------------------------------------	   
	...
	public ModelAndView newCreditCardTransc(@Valid @ModelAttribute("ccTrsc") authCreditCard ccTrsc,
		...
		synchronized (this) {
			...
			if (!flag) { // return to the current page
				...
				CreditCardDetails retCC = JerseyClientPost.ccCheck(cc); // Static method to call the remote RESTful Web
																		// Service
				System.out.println(retCC);
				boolean ccFlag = Boolean.valueOf(retCC.getCC_validity());

				// boolean ccFlag = helper.checkCC(Integer.toString(id_ccEmail), ccNumber,
				// ccCVV);
				...
				if (dblAmount < (currUserAmount - 50)) { // A threshold of minimum 50 units are required
					...
					System.out.println(
							"Email-ID: " + ccEmail + "Credit Card Number: " + ccNumber + ", Amount: " + dblAmount);
				}
				// ------------------------------------------------------------
				model.addAttribute("ccTrsc", ccTrsc);
...
//---------------------------------------------------------------------------------------------
	...
	public ModelAndView displayStatement(@Valid @ModelAttribute("displDat") displDate dspl, BindingResult result,
	...
	public ModelAndView execTransaction(@Valid @ModelAttribute("trsc") Transaction trsc, BindingResult result,
			HttpServletRequest request, ModelMap model) {
			...
			if (!flag) { // return to the current page
				...
				if (dblAmount < (currUserAmount - 50)) { // A threshold of minimum 50 units are required
					...
					if (action.equals("Debit From")) { // Debit?
						intRest = helper.transaction(emailRecv, remotUserAmount, emailSend, currUserAmount, dblAmount,
								id_recv, id_send);
						System.out.println("Debit From " + emailRecv + " To the logged User: " + emailSend + " Amount: "
								+ dblAmount);
					} else if (action.equals("Credit To")) { // Credit??
						intRest = helper.transaction(emailSend, currUserAmount, emailRecv, remotUserAmount, dblAmount,
								id_send, id_recv);
						System.out.println("Credit To from the logged User " + emailSend + " To " + emailRecv
								+ " Amount: " + dblAmount);
					}
				...
				model.addAttribute("trsc", trsc);
				model.addAttribute("status_message", "Transaction executed. Operation: " + action + ", " + emailSend
						+ ", " + emailRecv + ",  " + String.valueOf(dblAmount));
				return new ModelAndView("transaction");
			}
	...
	public ModelAndView newAccountCreated(@Valid @ModelAttribute("newUser") User newUser, BindingResult result,
			HttpServletRequest request, ModelMap model) {
...
//-------------------------------------------------------------------------------------------------
	...
	public ModelAndView welcomePage(User user, ModelMap model) {
		/*
		 * try { int val = (Integer) (Object) user; //clientObj; if (val==-1) {
		 * 
		 * model.addAttribute("user", new User()); return new ModelAndView("user",
		 * "command", new User()); } } catch (Exception e) { return new
